<template>
  <label
    class="h-checkbox-button"
    :class="[
        size ? 'h-checkbox-button--' + size : '',
  ...
  >
    <input
      ...
      class="h-checkbox-button__original"
      type="checkbox"
      :name="name"
      :disabled="isDisabled"
    ...
    <span
      class="h-checkbox-button__inner"
...
export default {
  ...
  methods: {
    ...
    handleChange(ev) {
      ...
      this.$nextTick(() => {
        if (this._checkboxGroup) {
          this.dispatch('HCheckboxGroup', 'change', [this._checkboxGroup.value])
        }
      })
    }
  },
